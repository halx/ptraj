...
extern int transformImgDist(actionInformation *, double *, double *, double *,
                            double *, int);
extern int transformOrderParameter(actionInformation *, double *, double *,
				   double *, double *, int);
...
/*
 * some data structures for memory managment
 */

typedef struct _sdDynData {
  int size;
  double *data;
} sdDynData;

typedef struct _dDynData {
  int *size;
  double **data;
} dDynData;
...
#define TRANSFORM_DIFFUSION_DIRECTIONS		\
  X(x),						\
    X(y),					\
    X(z),					\
    X(xy),					\
...
typedef enum _transformDiffusionType {
#define X(dir) DIFFUSION_##dir
  TRANSFORM_DIFFUSION_DIRECTIONS
#undef X
} transformDiffusionType;

const char transformDiffusionTypeMap[][4]  = {
#define X(dir) #dir
  TRANSFORM_DIFFUSION_DIRECTIONS
#undef X
};
...
#define INITIALIZE_transformDiffusionInfo(_p_) \
  ...
  _p_->mask           = NULL; \
  _p_->mask2          = NULL; \
  _p_->outputFilename = NULL; \
  _p_->outputAverDist = NULL; \
  _p_->outputNumWat   = NULL; \
  _p_->output         = NULL; \
  _p_->outputad       = NULL; \
  _p_->outputnw       = NULL
...
/*
 *  ACTION: TRANSFORM_MRT
 */

#define TO_MB (1024 * 1024)
#define MRT_INCSIZE 100         /* dynamic memory control */
#define MRT_INCSIZE2 10         /* dynamic memory control */

typedef struct _transformMRTInfo {
  int numberSoluteSites;
  int nStar;                    /* Nsteps not counted as having left/entered */
  int wSize;                    /* window size in steps */
  int nOffset;                  /* offset between windows */
  int idxMaxWin;                /* maximum number of parallel windows */
  int ncWin;                    /* number of parallel windows (grows to idxMaxWin) */
  int ntor;                     /* current window counter */
  int timeStep;                 /* frame counter */
  int allocMem;
  int memCap0;                  /* dynamic memory control */
  int *memCap1;                 /* dynamic memory control */
  int **memCap2;                /* dynamic memory control */
  int **soluteMask;             /* actually storage for indexes from mask */
  int *soluteMask2;             /* if MRT to another solute is requested */
  int *solventMask;
  int *solventMask2;            /* if sitemask is single molecule */
  int *in_;                     /* helper array: actual storage of time inside */
  int *out_;                    /* helper array: actual storage of time outside */
  int **inCount;                /* time waters are inside */
  int **outCount;               /* time waters are outside */
  int *tCounter;                /* length of each timeInside */
  int **timeInside;             /* time intervals of ANY water inside */
  int *nWater;                  /* storage for waters inside in each time steps */
  int ***nWater0;               /* storage for waters inside in each time steps */
  int *mem2_;                   /* helper array: dynamic memory managment */
  int *nwc_;                    /* helper array: nwCounter */
  int **nw_;                    /* helper array: nWater */
  int *nVisits_;                /* helper array */
  int **nwCounter0;             /* length of each nWater */
  int **nVisits;                /* visit counter for acf */
  double lowerCutoff;           /* define lower cutoff for inside region */
  double upperCutoff;           /* define upper cutoff for inside region */
  double *acf_;                 /* helper array */
  double **acf;                 /* accumulated acf data */
  char *autoCorr;               /* filename for acf output */
  char *filename;               /* filename for survival function output */
} transformMRTInfo;

#define INITIALIZE_transformMRTInfo(_p_) \
  _p_->numberSoluteSites = 0; \
  _p_->nStar             = 0; \
  _p_->wSize             = 0; \
  _p_->nOffset           = 0; \
  _p_->idxMaxWin         = 0; \
  ...
  _p_->memCap2           = NULL; \
  _p_->soluteMask        = NULL; \
  _p_->soluteMask2       = NULL; \
  _p_->solventMask       = NULL; \
  _p_->solventMask2      = NULL; \
  _p_->in_               = NULL; \
  _p_->out_              = NULL; \
  _p_->inCount           = NULL; \
  _p_->outCount          = NULL; \
...
/*
 *  ACTION: TRANSFORM_IMGDIST
 */

typedef struct _transformImgDistInfo {
  int timeStep;                 /* frame counter */
  int numberActiveAtoms;        /* atoms from the mask */
  int *mask;
  int *activeAtoms;
  double min;                   /* minimum distance */
  double threshold;      /* distances shorter than this value will be stored */
  char *filename;
  FILE *outFile;
} transformImgDistInfo;

#define INITIALIZE_transformImgDistInfo(_p_) \
  _p_->timeStep          = 0;                \
  _p_->numberActiveAtoms = 0;                \
  _p_->mask              = NULL;             \
  _p_->activeAtoms       = NULL;             \
  _p_->min               = 1E10;             \
  _p_->threshold         = 0.0;             \
  _p_->filename          = NULL;             \
  _p_->outFile           = NULL
...
/*
 *  ACTION: TRANSFORM_DENSITY
 */

#define MAX_ATN 10
#define MAX_LINE 256

typedef enum _transformDensityCoord { 
  DENSITY_X,
  DENSITY_Y,
  DENSITY_Z,
} transformDensityCoord;

typedef enum _transformDensityProp { 
  DENSITY_NUMBER,
  DENSITY_MASS,
  DENSITY_CHARGE,
  DENSITY_ELECTRON,
} transformDensityProp;

typedef struct _atomNameElec {
  int nElec;
  char atomName[MAX_ATN+1];
} atomNameElec;

typedef struct _transformDensityInfo {
  int nSlices;                  /* resolution */
  int nData;                    /* number of individual density data */
  int timeStep;                 /* frame counter */
  int **masks;                  /* calculate density for these masks */
  double delta;                 /* resolution */
  double **prop;                /* number, mass or charge density */
  char *filename;               /* output file name for data */
  char *eFile;                  /* input file name for number of electrons */
  atomNameElec *eTable;         /* atomname - number of electron table */
  dDynData *minusSlices;        /* to hold density data along - coordinate */
  dDynData *plusSlices;         /* to hold density data along + coordinate */
} transformDensityInfo;

#define INITIALIZE_transformDensityInfo(_p_) \
  _p_->nSlices           = 0;                \
  _p_->nData             = 0;                \
  _p_->timeStep          = 0;                \
  _p_->masks             = NULL;             \
  ...
  _p_->eTable            = NULL;             \
  _p_->plusSlices        = NULL;             \
  _p_->minusSlices       = NULL;             \
  _p_->filename          = NULL;             \
  _p_->eFile             = NULL


/*
 *  ACTION: TRANSFORM_PAIRDIST
 */

#define PAIRDIST_DISTDEF  25.0

typedef struct _transformPairDistInfo {
  int timeStep;                 /* frame counter */
  int numberActiveAtoms1;       /* atoms from the mask1 */
  int numberActiveAtoms2;       /* atoms from the mask2 */
  int ub1;
  int ub2;
  int *mask1;
  int *mask2;
  ...
  int *activeAtoms2;
  double delta;                 /* resolution */
  double maxDist;               /* maximum anticipated distance in mask */
  char *filename;
  FILE *outFile;
  sdDynData *histogram;
  sdDynData *histoTemp;
  sdDynData *histoVariance;
} transformPairDistInfo;

#define INITIALIZE_transformPairDistInfo(_p_) \
  _p_->timeStep           = 0;                \
  _p_->numberActiveAtoms1 = 0;               \
  _p_->numberActiveAtoms2 = 0;               \
  _p_->mask1              = NULL;            \
  _p_->mask2              = NULL;            \
  _p_->activeAtoms1       = NULL;            \
  _p_->activeAtoms2       = NULL;            \
  _p_->delta              = 0.1;             \
  _p_->maxDist            = 0.0;             \
  _p_->histogram          = NULL;            \
  _p_->histoTemp          = NULL;            \
  _p_->histoVariance      = NULL;            \
  _p_->filename           = NULL;            \
  _p_->outFile            = NULL


/*
 *  ACTION: TRANSFORM_ORDERPARAMETER
 */

#define ORDPRM_INCSIZE  20
#define ORDPRM_MAXBOND  3.0
#define ORDPRM_MAXBOND2 1.5
#define ORDPRM_MAXDIST  10.0


typedef double dVector[3];

typedef enum _transformOrderParameterCoord { 
  ORDPRM_X,
  ORDPRM_Y,
  ORDPRM_Z,
} transformOrderParameterCoord;

typedef struct _ordParaAtoms {
  int dbond;
  int *index;
} ordParaAtoms;

typedef struct _ordParaDynData {
  int size;
  ordParaAtoms *data;
} ordParaDynData;

typedef struct _transformOrderParameterInfo {
  int timeStep;                 /* frame counter */
  int numberActiveAtoms;
  int numberActiveAtoms2;
  int numberGroups;
  int *tailStart;
  int *tailEnd;
  int *unsatMask;
  ...
  double **ordParVariance;
  FILE *outFile;
  FILE *tailDistFile;
  sdDynData *histogram;
  sdDynData *histoTemp;
  sdDynData *histoVariance;
  ordParaDynData *indexes;
} transformOrderParameterInfo;

#define INITIALIZE_transformOrderParameterInfo(_p_) \
  _p_->timeStep          = 0;                \
  _p_->numberActiveAtoms = 0;                \
  _p_->numberActiveAtoms2= 0;                \
  _p_->numberGroups      = 0;                \
  ...
  _p_->_ophelper         = NULL;             \
  _p_->_ophelper2        = NULL;             \
  _p_->orderParameter    = NULL;             \
  _p_->ordParVariance    = NULL;             \
  _p_->outFile           = NULL;             \
  _p_->tailDistFile      = NULL;             \
  _p_->histogram         = NULL;             \
  _p_->histoTemp         = NULL;             \
  _p_->histoVariance     = NULL;             \
  _p_->indexes           = NULL

/* ====== above added by Hannes H Loeffler, STFC Daresbury ====== */
