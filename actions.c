			  ...
			  double *box, int mode)
{
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    fprintf(stdout, "              using a time interval of %f\n", action->darg1);
     ...
     double totalMass, max;
     double dx, dy, dz;
     double midx, midy, midz;
     double sumtr, sumxx, sumyy, sumzz, sumxy, sumxz, sumyz;
     double tr, xx, yy, zz, xy, xz, yz;
     ...
     sumtr = sumxx = sumyy = sumzz = sumxy = sumxz = sumyz = 0.0;
     max = 0.0;
     ...
     sumtr = sqrt(sumtr / totalMass);
     sumxx = sumxx / totalMass;
     sumyy = sumyy / totalMass;
     sumzz = sumzz / totalMass;
     sumxy = sumxy / totalMass;
     sumxz = sumxz / totalMass;
     sumyz = sumyz / totalMass;

     max = sqrt(max);

     ptrajfprintf(radiusInfo->outFile,
                  "%10.2f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n",
                  (double) action->iarg2 * action->darg1,
                  sumtr, sumxx, sumyy, sumzz, sumxy, sumxz, sumyz, max);
...
/* ====== added by Hannes H Loeffler, STFC Daresbury ====== */

/*
 * helper routine to dynamically allocate memory by a fixed amount
 *
 * written by Hannes H. Loeffler
 *
 */
		  ...
		  int *cur_size, int inc_size)
{
  ...
  if (need >= *cur_size) {
	...
	ptr = memset(ptr, (char) 0, tsize * inc_size);
	...
	*cur_size = inc_size;
	...
	error("increaseMem", "Error in reallocating of %i bytes",
	      tsize * (*cur_size + inc_size) );
      } else {
	/* we don't know data type of *ptr: do pointer arithmetic "by hand" */
	memset( (char *) ptr + tsize * *cur_size, (char) 0, tsize * inc_size);
...
/*
 * reconstruct full mask from a compressed mask
 *
 * written by Hannes H. Loeffler
 *
 */

int *reconstructMask (int *comprMask, int *fullMask, int n)
{
  int i, j;
...
/* simple helper function to find comment lines */
...
/*
 * a numerically stable online algorithm to compute mean and variance
 *
 * this function is supposed to be executed for every time step
 * the variance must be divided by (n(total) - 1) after the last time step
 *
 * written by Hannes H. Loeffler
 *
 */

void onlineAvVar (double *mean, double *var, const double x, const int nstep)
{
  double delta;

  delta = x - *mean;
  *mean += delta / (double) nstep;
  *var += delta * (x - *mean);
}


/*
 * helper routines for vector calculations
 *
 * written by Hannes H. Loeffler
 *
 */

void vecCreate(dVector r, double x, double y, double z)
{
  r[0] = x;
  r[1] = y;
  r[2] = z;
...
double vecLength(dVector r)
{
  return sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);
}

void vecScalarMult(dVector r, double a, dVector v)
{
  r[0] = a * v[0];
  r[1] = a * v[1];
  r[2] = a * v[2];
...
void vecSubtraction(dVector r, dVector v, dVector w)
{
  r[0] = v[0] - w[0];
  r[1] = v[1] - w[1];
  r[2] = v[2] - w[2];
...
void vecCrossProduct(dVector r, dVector v, dVector w)
{
  r[0] = v[1] * w[2] - v[2] * w[1];
  r[1] = v[2] * w[0] - v[0] * w[2];
  r[2] = v[0] * w[1] - v[1] * w[0];
...
void calculateMSD(transformDiffusionInfo *info, double *box,
		  ...
		  transformDiffusionType type)
{
  double xx, yy, zz;
  double delx, dely, delz, sum = 0.0, sum2 = 0.0;
  ...
  case DIFFUSION_xy:
    sum = delx*delx + dely*dely;
    sum2 = xx*xx + yy*yy;
    break;

  case DIFFUSION_xz:
    sum = delx*delx + delz*delz;
    sum2 = xx*xx + zz*zz;
    break;

  case DIFFUSION_yz:
    sum = dely*dely + delz*delz;
    sum2 = yy*yy + zz*zz;
    break;

  case DIFFUSION_xyz:
    sum = delx*delx + dely*dely + delz*delz;
    sum2 = xx*xx + yy*yy + zz*zz;
    break;

  default:
    error("transformDiffusion", "Unknown diffusion type!\n");
  }

  info->distance[idx2] = sum;
  info->dSum1[idx2] += sum2;	  /* sum r^2 */
  info->dSum2[idx2] += sqrt(sum2); /* sum r */
 ...
 *
 *  Supplementary routines:
 *    calculateMSD --- actual MSD calculation
 *
 *  rewritten by Hannes H. Loeffler
 *    2007: Lab. of Molecular Design, ICMB, the University of Tokyo, Japan
 *    2008-2011: STFC Daresbury Laboratory, UK
 *
 ******************************************************************************/
		       ...
		       double *box, int mode)
{
  ...
  int i, j, currentAtom, nInside, found;
   ...
   *  iarg2: 
   *    0 -- default
   *    1 -- MSD for center of mass
   *    2 -- print to nwout
   *  iarg3: 
   *    type = direction of MSD
   *  carg1:
   *    the transformDiffusionInfo structure 
   *
   */
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    info = (transformDiffusionInfo *) safe_malloc(sizeof(transformDiffusionInfo));
    ...
    if (mask == NULL) {
      fprintf(stderr, "WARNING in ptraj(), DIFFUSION: no mask specified. "
	      "Ignoring command!\n");
      safe_free(info);
      return -1;
    } else {
    ...
    if (info->timePerFrame < 0) {
      fprintf(stderr, "WARNING in ptraj(), DIFFUSION: diffusion time per "
	      "frame incorrectly specified. Ignoring command!\n");
      safe_free(info);
      return -1;
    }
    ...
    action->iarg3 = DIFFUSION_xyz;

    for (i = 0; i < sizeof(transformDiffusionTypeMap) /
	   sizeof(transformDiffusionTypeMap[0]); i++) {
      if (argumentStackContains(argumentStackPointer,
				(char *) transformDiffusionTypeMap[i]) ) {
	action->iarg3 = i;
	break;
      }
    }
  ...
  if (mode == PTRAJ_STATUS) {
    fprintf(stdout, "  DIFFUSION\n");
    fprintf(stdout, "      Diffusion in %s direction\n",
	    transformDiffusionTypeMap[action->iarg3]);
    fprintf(stdout, "      The atoms in mask1 are ");
    printAtomMask(stdout, info->mask, state);
    fprintf(stdout, "\n");

    if (action->iarg2 == 1) {
      fprintf(stdout, "      Center of mass diffusion of atoms in mask1 will "
	      "be computed\n");
    } else if (action->iarg2 == 2) {
      fprintf(stdout,
	      "      Atoms in mask2 in the range %7.3f to %7.3f Angstrom will "
	      "be used\n",
	      sqrt(info->lowerCutoff), sqrt(info->upperCutoff));
      fprintf(stdout, "      The atoms in mask2 are ");
      printAtomMask(stdout, info->mask2, state);
      fprintf(stdout, "\n");
    }
    ...
    if (action->iarg2 == 2) {
      fprintf(stdout, "      The number of atoms in the shell will be dumped "
	      "to %s\n",
	      info->outputNumWat);
    }
  ...
  if (info->dx == NULL) {
	...
	if (info->mask == NULL || info->mask[i] == 1) {
      ...
      info->nInside = safe_malloc(sizeof(int) * state->atoms);
    ...
    if (!openFile(&info->output, info->outputFilename, "w") ) {
      fprintf(stderr,
	      "WARNING in ptraj(), DIFFUSION: Cannot open diffusion output "
	      "file %s\n",
	      info->outputFilename);
    }
    ...
    fprintf(info->output, "#time x y z %s",
	    transformDiffusionTypeMap[action->iarg3]);

    if (action->iarg1) {
      for (i = 0; i < info->activeAtoms; i++) {
	fprintf(info->output, " x%i y%i z%i r%i", i, i, i, i);
      }
    }
  ...
  if (action->iarg2 == 0) {	/* all */
    for (currentAtom = 0, i = 0; i < state->atoms; i++) {
      if (info->mask[i] == 1) {
	calculateMSD(info, state->box, x, y, z, i, currentAtom, action->iarg3);
	currentAtom++;
      }
    }
  } else if (action->iarg2 == 1) {  /* center of mass */
    ...
    calculateMSD(info, state->box, X, Y, Z, 0, 0, action->iarg3);
  } else if (action->iarg2 == 2) {  /* region based */
	...
	for (j = 0; j < state->atoms; j++) {
	  if (info->mask2[j] == 1) {
	    ...
	    dist2 = calculateDistance2(0, 1, XX, YY, ZZ, box, ucell, recip, 0.0, 0);
	...
	if (minDist > info->lowerCutoff && minDist < info->upperCutoff) {
	  info->nInside[i] = 1;
	  calculateMSD(info, state->box, x, y, z, i, i, action->iarg3);
	}
      }
    }
  }
  ...
  if (action->iarg2 == 0) {	/* all */
    ...
    if (nInside == 0)
      error("diffusion()", "No atoms of mask 1 left for processing.  "
	    "Aborting!\n");
...
/*
 * calculate the actual correlation functions for mean residence times
 * based on the survival function (RW Impey, PA Madden and IR McDonald,
 *   JPhysChem 87(1983), 5071) and the autocorrelation function (optional)
 *
 * written by Hannes H. Loeffler
 *
 */

void inout_MRT (transformMRTInfo *info, int inside, int newWin,
		int *nWat, int t, int i, int j)
{
  int k, l, m, theta;
  ...
  if (inside) {  /* inside */
    if (info->outCount[i][j] <= info->nStar) {
      ...
      info->timeInside[i] =
	increaseMem(info->timeInside[i],  sizeof(int),
		    ++info->tCounter[i], &info->memCap1[i], MRT_INCSIZE);
 ...
 *
 *  Supplementary routines:
 *    inout_MRT --- actual correlation function calculation
 *    calculateDistance2 --- calculate a distance**2 and do imaging (ptraj.c)
 *    increaseMem --- dynamically increase memory by a fixed amount
 *    reconstructMask --- reconstruct mask from a compress mask
 *
 *  written by Hannes H. Loeffler
		 ...
		 double *box, int mode)
{
  ...
  int i, j, jj, k, kk, l, memSize, memSize2;
  int t, B, intLen, longestInterval, inside;
  int siteatoms;
  int newWin, nWat, nWin;
  int *atomMask, *memCap;

  char *mask, *buffer;

  double dist2, minDist, total_mass0, total_mass1, tcorr, toffset;;
  double X[2], Y[2], Z[2];
  double ucell[9], recip[9];
   ...
   *  iarg2: single molecule reference
   *  iarg3: MRT to solvent or solute
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    info->allocMem += strlen(info->filename);
    
    if (info->filename == NULL) {
      fprintf(stderr, "WARNING in ptraj(), MRT: no output file specified. "
	      "Ignoring command!\n");
      safe_free(info);
      return -1;
    }
    ...
    info->nStar = (int) floor(
      argumentStackKeyToDouble(argumentStackPointer, "tstar", 0.0) /
      action->darg1 + 0.5);

    /* lower and upper distance limits */
    info->lowerCutoff =		/* default prevents binning of 0 distances */
      argumentStackKeyToDouble(argumentStackPointer, "lower", 0.01);

    info->upperCutoff =		/* upper limit approx. RDF minimum */
      argumentStackKeyToDouble(argumentStackPointer, "upper", 3.5);
    ...
    tcorr = argumentStackKeyToDouble(argumentStackPointer, "tcorr", 400.0);
    info->wSize = (int) floor(tcorr / action->darg1 + 0.5);

    toffset = argumentStackKeyToDouble(argumentStackPointer, "toffset", 10.0);
    info->nOffset = (int) floor(toffset / action->darg1 + 0.5);

    if (info->nOffset < 1 || info->nOffset > info->wSize) {
      fprintf(stderr, "WARNING in ptraj(), MRT: toffset must be in the range "
	      "from %8.3f to %8.3f. Ignoring command!\n",
	      action->darg1, info->wSize * action->darg1);
      safe_free(info);
      return -1;
    }

    if (fmod(tcorr, toffset) != 0) {
      fprintf(stderr, "WARNING in ptraj(), MRT: tcorr must be multiple of "
	      "toffset. Ignoring command!\n");
      safe_free(info);
      return -1;
    }
    ...
    /*
     * check if second species are either solvent or another solute
     * if it is a solute we use the mask as a single site
     */

    /* check if MRT to another solute is requested */
    mask = argumentStackKeyToString(argumentStackPointer, "solute", NULL);

    if (mask != NULL) {
      atomMask = processAtomMask(mask, state);

      if (atomMask == NULL) {
	fprintf(stderr, "WARNING in ptraj(), MRT: no solute mask specified. "
		"Ignoring command!\n");
	safe_free(info);
	return -1;
      }
	...
	if (atomMask[j] > 0) {
	  info->soluteMask2 =
	    increaseMem(info->soluteMask2, sizeof(int), i+2, &memSize, MRT_INCSIZE);
	  info->soluteMask2[i] = j;
	  i++;
	}
      }

      info->soluteMask2[i] = -1;
      action->iarg3 = 1;

      info->allocMem += sizeof(info->soluteMask2);
    } else {
      /* check for solvent mask */
      mask = argumentStackKeyToString(argumentStackPointer, "solvent", NULL);

      if (mask == NULL) {
	if (!state->solventMask) {
	  fprintf(stderr, "WARNING in ptraj(), MRT: no solvent found! "
		  "Ignoring command!\n");
	  safe_free(info);
	  return -1;
	}
      
	info->solventMask = safe_malloc(sizeof(int)  * state->atoms);

	for (i = 0; i < state->atoms; i++) {
	  info->solventMask[i] = state->solventMask[i];
	}
      } else {
	info->solventMask = processAtomMask(mask, state);
      ...
      if (info->solventMask == NULL) {
	fprintf(stderr, "WARNING in ptraj(), MRT: no solvent mask specified. "
		"Ignoring command!\n");
	safe_free(info);
	return -1;
      }
    ...
    /*
     * Process reference sites.  Currently we have:
	...
	if (atomMask[j] > 0) {
	  info->soluteMask =
	    increaseMem(info->soluteMask, sizeof(*info->soluteMask), i+2,
			&memSize, MRT_INCSIZE);

	  /* store only indexes of "active" atoms */
	  info->soluteMask[i] = safe_malloc(sizeof(int) * 2);
	  memSize2 += 2 * sizeof(info->soluteMask);
    ...
    /* treat the reference mask as a single entity? */
    action->iarg2 = argumentStackContains(argumentStackPointer, "onemol");

    if (action->iarg2) {
      info->solventMask2 = safe_malloc(sizeof(int) * state->solventMolecules);
      info->allocMem += sizeof(info->solventMask2);
    }

    /* try to read rest of command line as individual masks
       onemol is treated the same way just that it is a single mask */
    if (!siteatoms) {
      for (i = 0;
	   (buffer = getArgumentString(argumentStackPointer, NULL)) != NULL;
	   i++) {
	info->soluteMask =
	  increaseMem(info->soluteMask, sizeof(*info->soluteMask), i,
		      &memSize, MRT_INCSIZE);
	memCap =
	  ...
	  if (atomMask[k] > 0) {
	    info->soluteMask[i] = (int *)
	      increaseMem(info->soluteMask[i],
			  sizeof(int), j + 2, &memCap[i], MRT_INCSIZE);
    ...
    info->numberSoluteSites = i;

    if (info->numberSoluteSites < 1) {
    ...
    info->allocMem += sizeof(int) * info->numberSoluteSites
      * state->solventMolecules;
    info->allocMem += sizeof(int) * info->numberSoluteSites
      * state->solventMolecules;
    ...
    info->inCount =
      safe_malloc(sizeof(*info->inCount) * info->numberSoluteSites);
    info->outCount =
      safe_malloc(sizeof(*info->outCount) * info->numberSoluteSites);

    info->allocMem += sizeof(info->inCount) * info->numberSoluteSites;
    info->allocMem += sizeof(info->outCount) * info->numberSoluteSites;
    ...
    info->tCounter =
      safe_malloc(sizeof(int) * info->numberSoluteSites);
    info->allocMem += sizeof(info->tCounter) * info->numberSoluteSites;
    ...
    info->memCap1 =
      safe_malloc(sizeof(int) * info->numberSoluteSites);
    info->allocMem += sizeof(info->memCap1) * info->numberSoluteSites;

    /* preallocate memory for first dimension */
    info->timeInside =
      safe_malloc(sizeof(*info->timeInside) * info->numberSoluteSites);
    info->allocMem += sizeof(info->timeInside) * info->numberSoluteSites;
	...
	safe_malloc(sizeof(double) * info->numberSoluteSites * info->wSize);
      ...
      info->nWater0 =
	safe_malloc(sizeof(info->nWater0) * info->numberSoluteSites);

      info->nwCounter0 =
	safe_malloc(sizeof(*info->nwCounter0) * info->numberSoluteSites);

      info->memCap2 = safe_malloc(sizeof(*info->memCap2) * info->numberSoluteSites);

      info->nVisits = safe_malloc(sizeof(*info->nVisits) * info->numberSoluteSites);

      info->acf = safe_malloc(sizeof(*info->acf) * info->numberSoluteSites);

      info->allocMem += 5 * sizeof(double) * info->numberSoluteSites;
  ...
  if (mode == PTRAJ_STATUS) {
    ...
    fprintf(stdout,
            "      Molecules considered in the range %6.2f to %6.2f Angstrom\n",
            sqrt(info->lowerCutoff), sqrt(info->upperCutoff) );

    if (action->iarg3) {
      atomMask = safe_malloc(sizeof(int) * state->atoms);
      atomMask = reconstructMask(info->soluteMask2, atomMask, state->atoms);

      fprintf(stdout, "      Solute atom selection is ");
      printAtomMask(stdout, atomMask, state);
      fprintf(stdout, "\n");

      safe_free(atomMask);
    } else {
      fprintf(stdout, "      Solvent atom selection is ");
      printAtomMask(stdout, info->solventMask, state);
      fprintf(stdout, "\n");
    }

    if (action->iarg2)
      fprintf(stdout, "      Solute mask is treated as single molecule\n");

    fprintf(stdout, "      Solute atom reference selection is (%i site%s)\n",
	    info->numberSoluteSites,
	    info->numberSoluteSites > 1 ? "s" : "");

    for (i = 0; i < info->numberSoluteSites; i++) {
      atomMask = safe_malloc(sizeof(int) * state->atoms);
      atomMask = reconstructMask(info->soluteMask[i], atomMask, state->atoms);
    ...
    fprintf(stdout, "MRT: dumping correlation data to output file%s\n",
	    info->autoCorr ? "s" : "");
    ...
    for (i = 0; i < info->numberSoluteSites; i++) {
      ...
      printAtomMask(outFile, atomMask, state);
      fprintf(outFile, " ");
    ...
    fprintf(stdout, "\nMRT: longest time span solvent/solute based on the survival functions\nMRT: spent near\n");
    ...
    for (i = 0; i < info->numberSoluteSites; i++) {
      ...
      for (j = 0; j < info->tCounter[i]; j++) {
        ...
        if (info->timeInside[i][j] > l)
          l = info->timeInside[i][j];
      }

      fprintf(stdout, "MRT:   site %5i = %11.2f\n",
	      i+1, l * action->darg1);
    }
	...
	printAtomMask(acfFile, atomMask, state);
	fprintf(acfFile, " ");
	...
	safe_free(atomMask);
      ...
      if (info->timeStep < info->wSize)
	l = info->timeStep;
      else
	l = info->wSize;

      for (t = 0; t < l; t++) {  /* time loop */
    ...
    fprintf(stdout, "MRT: %8.2f MB of memory allocated\n",
            (double) info->allocMem / TO_MB);
    ...
    fprintf(stdout, "MRT: %8.2f MB of memory allocated dynamically\n",
            (double) memSize / TO_MB);

    fprintf(stdout, "MRT: %8.2f MB total memory allocated\n",
  ...
  for (i = 0; i < info->numberSoluteSites; i++) {  /* i loop */
    ...
    if (!action->iarg2) {	/* individual references */
      X[0] = Y[0] = Z[0] = total_mass0 = 0.0;
	...
	for (j = 0; (l = info->soluteMask[i][j]) >= 0; j++) {
	  ...
	  dist2 = calculateDistance2(0, 1, X, Y, Z, box, ucell, recip,
				     0.0, action->iarg1);
    ...
    if (!action->iarg3) {	/* solvent molecules */
	...
	inout_MRT(info, inside, newWin, &nWat, t, i, j);
      }
    } else {			/* solute molecules */
      X[1] = Y[1] = Z[1] = total_mass1 = 0.0;

      for (jj = 0; (k = info->soluteMask2[jj]) >= 0; jj++) {
	X[1] += state->masses[k] * x[k];
	Y[1] += state->masses[k] * y[k];
	Z[1] += state->masses[k] * z[k];
	total_mass1 += state->masses[k];
      }
...
/** ACTION ROUTINE ************************************************************
 *
 *  transformDensity()   --- calculate density along a chosen cartesian
 *                           coordinate (assumes rectangular box)
 *
 *  Supplementary routines:
 *
 *    
 *  written by Hannes H. Loeffler
 *    2009-2011: STFC Daresbury Laboratory, UK
 *
 *****************************************************************************/

int transformDensity(actionInformation *action, double *x, double *y, double *z, 
		     double *box, int mode)
{
  stackType **argumentStackPointer;
  ptrajState *state;
  transformDensityInfo *info;
  dDynData *minus, *plus;

  FILE *outFile, *eFile;

  int nElec, slice, nSlices, neLines;
  int i, j, k, l;
  int found;
  int mMemSize, mMemSize2, wMemSize, growBy;
  int *atomMask;
  const int MAX_CHAR = 10;

  char dch;
  char line[MAX_LINE], atomName[MAX_ATN+1], compName[MAX_ATN+1];
  char *buffer, *loc;

  double coord, dens, *coor;

  /*
   *  USAGE
   *
   *  density out <filename> [delta <resolution>] [x|y|z]
   *          [number|mass|charge|electron] [efile] <mask1> ... <maskN>
   *
   *  action argument usage:
   *
   *  carg1: a transformDensity structure (stored in actionInformation for
   *         repeated calls)
   *
   *  iarg1: direction; 0 = x, 1 = y, 2 = z
   *  iarg2: property; 0 = number, 1 = mass, 2 = charge,
   *                   3 = electron (only primitive implementation)
   *
   */
  ...
  if (mode == PTRAJ_SETUP) {	/* PTRAJ_SETUP */
    ...
    info = (transformDensityInfo *) safe_malloc(sizeof(transformDensityInfo));
    INITIALIZE_transformDensityInfo(info);
    ...
    if (info->filename == NULL) {
      fprintf(stderr, "WARNING in ptraj(), DENSITY: no output file specified. "
	      "Ignoring command!\n");
     safe_free(info);
      return -1;
    }

    /* density along x, y or z direction; z is default */
    action->iarg1 = DENSITY_Z;

    found = argumentStackContains(argumentStackPointer, "x");
    if (found)
      action->iarg1 = DENSITY_X;

    found = argumentStackContains(argumentStackPointer, "y");
    if (found)
      action->iarg1 = DENSITY_Y;

    found = argumentStackContains(argumentStackPointer, "z");
    if (found)
      action->iarg1 = DENSITY_Z;

    /* density by number, mass or charge; number is default  */
    action->iarg2 = DENSITY_NUMBER;

    found = argumentStackContains(argumentStackPointer, "number");
    if (found)
      action->iarg2 = DENSITY_NUMBER;

    found = argumentStackContains(argumentStackPointer, "mass");
    if (found)
      action->iarg2 = DENSITY_MASS;

    found = argumentStackContains(argumentStackPointer, "charge");
    if (found)
      action->iarg2 = DENSITY_CHARGE;

    found = argumentStackContains(argumentStackPointer, "electron");
    if (found)
      action->iarg2 = DENSITY_ELECTRON;

    /* optional file with electron numbers for each atomname */
    info->eFile =
      argumentStackKeyToString(argumentStackPointer, "efile", NULL);

    /* number of slices */
    info->delta =
      argumentStackKeyToDouble(argumentStackPointer, "delta", 0.5);
    ...
    for (i = 0;
	 ...
	 i++) {
      ...
      if ( (atomMask = processAtomMask(buffer, state)) == NULL) {
	fprintf(stderr, "WARNING in ptraj(), DENSITY: error in mask. "
		"Ignoring command!\n");
	safe_free(info);
	return -1;
      }
      ...
      info->masks =
	increaseMem(info->masks, sizeof(int *), i+1, &mMemSize,
		    MRT_INCSIZE2);
	...
	if (atomMask[k] > 0) {
	  info->masks[i] =
	    increaseMem(info->masks[i], sizeof(int), j+2, &mMemSize2,
			MRT_INCSIZE);
      ...
      info->masks[i][j] = -1;	/* tag end of array */
    }

    if ( (info->nData = i) < 1) {
      fprintf(stderr, "WARNING in ptraj(), DENSITY: no mask specified. "
	      "Ignoring command!\n");
      safe_free(info);
      return -1;
    }
    ...
    info->minusSlices = safe_malloc(sizeof(dDynData));
    info->plusSlices = safe_malloc(sizeof(dDynData));

    info->minusSlices->data = safe_malloc(sizeof(*info->minusSlices->data ) *
					  info->nData);
    info->minusSlices->size = safe_malloc(sizeof(double) * info->nData);

    info->plusSlices->data = safe_malloc(sizeof( *info->plusSlices->data) *
					 info->nData);
    info->plusSlices->size = safe_malloc(sizeof(double) * info->nData);

    info->prop = safe_malloc(sizeof(*info->prop)  * info->nData);

    /* read file with electron number informtion */
    if (info->eFile != NULL) {
      /* read first time to count the number of entries */
      if ( (eFile = fopen(info->eFile, "r")) == NULL) {
	fprintf(stderr, "WARNING in ptraj(), DENSITY: "
		"couldn't open input file %s.\n", info->eFile);
	return -1;
      }

      neLines = 0;
      while (fgets(line, MAX_LINE, eFile) != NULL) {
	if (isComment(line) )
	  continue;
	...
	neLines++;
      ...
      fclose(eFile);
      info->eTable = safe_malloc(sizeof(atomNameElec) * neLines);

      /* read a second time to get the entries */
      if ( (eFile = fopen(info->eFile, "r")) == NULL) {
	fprintf(stderr, "WARNING in ptraj(), DENSITY: "
		"couldn't open input file %s.\n", info->eFile);
	return -1;
	...
	if ( (buffer = strtok(line, "=: ")) == NULL) {
	  fprintf(stderr, "WARNING in ptraj(), DENSITY: "
		  "malformed line #%i in %s.\n", j, info->eFile);
	  return -1;
	}

	strncpy(atomName, buffer,  MAX_ATN);
	...
	nElec = strtol(buffer, (char **) NULL, 10);

	strncpy(info->eTable[i].atomName, atomName, MAX_ATN);
	info->eTable[i].nElec = nElec;
    ...
    /* check which density is wanted and update data structures with property */
    for (i = 0; i < info->nData; i++) {
      wMemSize = 0;

      for (j = 0; (k = info->masks[i][j]) >= 0; j++) {
	info->prop[i] =
	  increaseMem(info->prop[i], sizeof(double), j+1, &wMemSize,
		      MRT_INCSIZE2);
	...
	case DENSITY_MASS:
	  info->prop[i][j] = state->masses[k];
	  break;

	case DENSITY_CHARGE:
	  info->prop[i][j] = state->charges[k];
	  break;

	case DENSITY_ELECTRON:
	  if (info->eFile == NULL) {  /* determine from atom names in topology
					 files, very primitive implementation:
					 assumes first char is element name */
	    switch(state->atomName[k][0]) {
	    case 'H':
	      nElec = 1;
	      break;
	    ...
	    default:
	      fprintf(stderr, "WARNING in ptraj(), DENSITY: electron density "
		      "for %s unknown!\n", state->atomName[k]);
	      safe_free(info);
	      return -1;
	    }

	    info->prop[i][j] = (double) nElec - state->charges[k];
	  } else {		/* determine from external source */

	    for (l = 0; l < neLines; l++) {
	      strncpy(compName, state->atomName[k], MAX_ATN);

	      if ( (loc = strchr(compName, ' ')) != NULL) {
		*loc = '\0';
	      }

	      found = 0;
	      if (strncmp(compName, info->eTable[l].atomName, MAX_ATN) == 0) {
		found = 1;
		info->prop[i][j] = (double) info->eTable[l].nElec - state->charges[k];
		break;
	      }
	    }

	    if (!found) {
	      fprintf(stderr, "WARNING in ptraj(), DENSITY: no entry found "
		      "for %s in %s!\n", state->atomName[k], info->eFile);
	      safe_free(info->eTable);
	      safe_free(info);
	      return -1;
	    }
	  }
  ...
  /* short cuts */
  info = (transformDensityInfo *) action->carg1;
  minus = info->minusSlices;
  plus = info->plusSlices;
  ...
  if (mode == PTRAJ_STATUS) {
    ...
    buffer = safe_malloc(sizeof(buffer) * (MAX_CHAR+1));

    switch (action->iarg2) {
    case DENSITY_NUMBER:
      strncpy(buffer, "number", MAX_CHAR);
      break;

    case DENSITY_MASS:
      strncpy(buffer, "mass", MAX_CHAR);
      break;

    case DENSITY_CHARGE:
      strncpy(buffer, "charge", MAX_CHAR);
      break;

    case DENSITY_ELECTRON:
      strncpy(buffer, "electron", MAX_CHAR);
      break;
    }

    fprintf(stdout, "  DENSITY: calculate %s density along %c axis\n", buffer, dch);
    safe_free(buffer);

    fprintf(stdout, "      Output will be written to %s\n", info->filename);

    if (info->eFile != NULL) {
      fprintf(stdout, "      Electron numbers read from %s\n", info->eFile);;
    }

    fprintf(stdout, "      Resolution is %7.3f\n", info->delta);
    fprintf(stdout, "      Atom selection%s\n", info->nData < 2 ? " is" : "s are");

    for (i = 0; i < info->nData; i++) {
      fprintf(stdout, "        %5i) ", i+1);

      atomMask = safe_malloc(sizeof(int) * state->atoms);
      atomMask = reconstructMask(info->masks[i], atomMask, state->atoms);
    ...
    fprintf(stdout, "PTRAJ DENSITY: dumping histogram data to output "
	    "file %s\n\n", info->filename);

    if (openFile(&outFile, info->filename, "w") == 0) {
      fprintf(stderr, "WARNING in ptraj(), DENSITY: couldn't open output "
	      "file %s.\n", info->filename);
      return -1;
    }

    /* negative slices */
    for (i = (info->nSlices - 1); i >= 0; i--) {
      coord = -((double) i + 0.5) * info->delta;
      fprintf(outFile, "%10.4f ", coord);

      for (j = 0; j < info->nData; j++) {
	dens = minus->data[j][i] / (info->delta * (double) info->timeStep);
	fprintf(outFile, "%10.3f", dens);
      }
    ...
    for (i = 0; i < info->nSlices; i++) {
      ...
      for (j = 0; j < info->nData; j++) {
	dens = plus->data[j][i] / (info->delta * (double) info->timeStep);
	fprintf(outFile, "%10.3f", dens);
      }
    ...
    for (i = 0; i < info->nData; i++) {
      safe_free(info->masks[i]);
      safe_free(info->prop[i]);
      safe_free(minus->data[i]);
      safe_free(plus->data[i]);
    }

    safe_free(info->masks);
    safe_free(info->prop);
    safe_free(minus->data);
    safe_free(minus->size);
    safe_free(minus);
    safe_free(plus->data);
    safe_free(plus->size);
    safe_free(plus);
  ...
  nSlices = (int) (box[action->iarg1] / info->delta);

  if (nSlices > info->nSlices) {
  ...
  for (i = 0; i < info->nData; i++) {
    ...
    /* allocate more memory if box size has increased */
    growBy = nSlices - minus->size[i] + MRT_INCSIZE2;

    minus->data[i] =
      increaseMem(minus->data[i], sizeof(double), nSlices, &minus->size[i],
		  growBy);
    plus->data[i] =
      increaseMem(plus->data[i], sizeof(double), nSlices, &plus->size[i],
		  growBy);

    /* fill minus and plus histograms */
    for (j = 0; (k = info->masks[i][j]) >= 0; j++) {
      /* slice the coordinates */
      if (coor[k] < 0) {
	slice = (int) abs(coor[k] / info->delta);

	if (slice < nSlices) {
	  minus->data[i][slice] += info->prop[i][j];
	} else {
	  fprintf(stderr, "WARNING in ptraj(), DENSITY: coordinate outside "
		  "range. Ignoring this data point!\n");
	  return 1;
	}
      } else if (coor[k] >= 0) {
	slice = (int) (coor[k] / info->delta);

	if (slice < nSlices) {
	  plus->data[i][slice] += info->prop[i][j];
	} else {
	  fprintf(stderr, "WARNING in ptraj(), DENSITY: coordinate outside "
		  "range. Ignoring this data point!\n");
	  return 1;
 ...
 *  written by Hannes H. Loeffler
 *    2007: Lab. of Molecular Design, ICMB, the University of Tokyo, Japan
 *    2008-2009: STFC Daresbury Laboratory, UK
 *
 *****************************************************************************/
		     ...
		     double *box, int mode)
{
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    if (mask == NULL) {
      ...
      info->mask = processAtomMask(mask, state);
      info->activeAtoms = safe_malloc(sizeof(int) * state->atoms);
    ...
    /* threshold */
    info->threshold =
      argumentStackKeyToDouble(argumentStackPointer, "threshold", 5.0);
...
/** ACTION ROUTINE ************************************************************
 *
 *  transformPairDist()   --- calculate pair-distance distribution function P(r)
 *
 *  Supplementary routines:
 *
 *    
 *  written by Hannes H. Loeffler
 *    2010-2011: STFC Daresbury Laboratory, UK
 *
 *****************************************************************************/

int transformPairDist(actionInformation *action, double *x, double *y, double *z, 
		      double *box, int mode)
{
  stackType **argumentStackPointer;
  ptrajState *state;
  transformPairDistInfo *info;
   ...
   *  action argument usage:
   *
   *  carg1: a transformPairDist structure (stored in actionInformation for
   *         repeated calls)
   */
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    info = (transformPairDistInfo *) safe_malloc(sizeof(transformPairDistInfo));
    INITIALIZE_transformPairDistInfo(info);
    ...
    if (mask == NULL) {
      fprintf(stderr, "WARNING in ptraj(), PAIRDIST: no mask specified. "
	      "Ignoring command!\n");
      safe_free(info);
      return -1;
    } else {
      info->mask1 = processAtomMask(mask, state);
      
      info->activeAtoms1 = safe_malloc(sizeof(int) * state->atoms);

      for (i = 0, j = 0; i < state->atoms; i++) {
	if (info->mask1[i] > 0) {
	  info->activeAtoms1[j] = i;
	  j++;
	}
      }

      info->numberActiveAtoms1 = j;
    }

    mask = argumentStackKeyToString(argumentStackPointer, "mask2", NULL);

    if (mask == NULL) {
      info->mask2 = info->mask1;
      info->activeAtoms2 = info->activeAtoms1;
      info->numberActiveAtoms2 = info->numberActiveAtoms1;
    } else {
      info->mask2 = processAtomMask(mask, state);
      
      info->activeAtoms2 = safe_malloc(sizeof(int) * state->atoms);

      for (i = 0, j = 0, jj = 0; i < state->atoms; i++) {
	if (info->mask2[i] > 0) {
	  for (ii = 0; ii < info->numberActiveAtoms1; ii++) {
	    if (info->activeAtoms1[ii] == i) {
	      jj++;
	    }
	  }

	  info->activeAtoms2[j] = i;
	  j++;
	}
      }

      if (jj != info->numberActiveAtoms1 && jj != 0) {
	fprintf(stderr, "WARNING in ptraj(), PAIRDIST: partially overlapping "
		"selections. Ignoring command!\n");
	safe_free(info);
	return -1;
      }

      if (jj == info->numberActiveAtoms1) {
	free(info->activeAtoms2);

	info->mask2 = info->mask1;
	info->activeAtoms2 = info->activeAtoms1;
	info->numberActiveAtoms2 = info->numberActiveAtoms1;
      } else {
	info->numberActiveAtoms2 = j;
      }
    }

    if (info->mask1 == info->mask2) {
      info->ub1 = info->numberActiveAtoms1 - 1;
      info->ub2 = info->numberActiveAtoms1;
    } else {
      info->ub1 = info->numberActiveAtoms1;
      info->ub2 = info->numberActiveAtoms2;
    }

    /* resolution */
    info->delta =
      argumentStackKeyToDouble(argumentStackPointer, "delta", 0.1);
    ...
    if (info->filename == NULL) {
      fprintf(stderr, "WARNING in ptraj(), PAIRDIST: no output file specified. "
	      "Ignoring command!\n");
      safe_free(info);
      return -1;
    }

    info->histogram = safe_malloc(sizeof(*(info->histogram) ) );
    info->histoTemp = safe_malloc(sizeof(*(info->histoTemp) ) );
    info->histoVariance = safe_malloc(sizeof(*(info->histoVariance) ) );

    info->histogram->size =  info->histoTemp->size =
      info->histoVariance->size = 0;
    size = (int) (PAIRDIST_DISTDEF / info->delta) + 1;

    info->histogram->data = increaseMem(info->histogram->data, sizeof(double),
					size, &(info->histogram->size), size);
    info->histoTemp->data = increaseMem(info->histoTemp->data, sizeof(double),
					size, &(info->histoTemp->size), size);
    info->histoVariance->data = increaseMem(info->histoVariance->data,
					    sizeof(double),
					    size, &(info->histoVariance->size),
					    size);
  ...
  info = (transformPairDistInfo *) action->carg1;

  if (mode == PTRAJ_STATUS) {
    fprintf(stdout, "  PAIRDIST: pair-distance distribution P(r) for mask ");
    printAtomMask(stdout, info->mask1, state);
    fprintf(stdout, "\n");

    if (info->mask2 != info->mask1) {
      fprintf(stdout, "                with mask ");
      printAtomMask(stdout, info->mask2, state);
      fprintf(stdout, "\n");
    }

    fprintf(stdout, "      Resolution is %6.2f Angstrom\n", info->delta);
    fprintf(stdout, "      P(r) data is written to %s\n", info->filename);

    if (openFile(&info->outFile, info->filename, "w") == 0) {
      fprintf(stderr, "WARNING in ptraj(), IMGDIST: couldn't open output file "
	      "%s.\n", info->filename);
      safe_free(info);
      return -1;
    }
  } else if (mode == PTRAJ_PRINT) {
    nstep = info->timeStep - 1;

    fprintf(info->outFile, "# pair-distance distribution P(r)\n");

    for (i = 0; i < info->histogram->size; i++) {
      d = ((double) i + 0.5) * info->delta;
      Pr = info->histogram->data[i] / info->delta;

      if (Pr > 0.0) {
	fprintf(info->outFile, "%10.4f %10.4f %10.7f\n", d, Pr,
		sqrt(info->histoVariance->data[i] / nstep) );
      }
    }
  } else if (mode == PTRAJ_CLEANUP) {
    safe_free(info->activeAtoms1);

    if (info->activeAtoms2 != info->activeAtoms1) {
      safe_free(info->activeAtoms2);
    }

    safe_free(info->histogram->data);
    safe_free(info->histogram);
    safe_free(info->histoTemp->data);
    safe_free(info->histoTemp);
    safe_free(info->histoVariance->data);
    safe_free(info->histoVariance);
    safe_free(info);
  }
  ...
  for (i = 0; i < info->ub1; i++) {
    ii = info->activeAtoms1[i]; /* extract indices */
    ...
    for (/* empty */; j < info->ub2; j++) {
      jj = info->activeAtoms2[j]; /* extract indices */

      /* calculate distance and put it in appropriate bin */
      dx = x[ii] - x[jj];
      dy = y[ii] - y[jj];
      dz = z[ii] - z[jj];

      bin = (int) (sqrt(dx*dx + dy*dy + dz*dz) / info->delta);

      info->histoTemp->data =
	increaseMem(info->histoTemp->data, sizeof(double), bin,
		    &(info->histoTemp->size),
		    bin - info->histoTemp->size + 50);

      info->histoTemp->data[bin] += 1.0;
    }
  }

  info->histogram->data =
    increaseMem(info->histogram->data, sizeof(double),
		info->histoTemp->size, &(info->histogram->size),
		info->histoTemp->size - info->histogram->size);

  info->histoVariance->data =
    increaseMem(info->histoVariance->data, sizeof(double),
		info->histoTemp->size, &(info->histoVariance->size),
		info->histoTemp->size - info->histoVariance->size);

  for (i = 0; i < info->histogram->size; i++) {
      onlineAvVar(&(info->histogram->data[i]),
		  &(info->histoVariance->data[i]),
		  info->histoTemp->data[i], nstep);

    info->histoTemp->data[i] = 0.0;
  }
...
/** ACTION ROUTINE ************************************************************
 *
 *  transformOrderParameter() --- calculate lipid order parameters in planar
 *                                membranes       
 *
 *  Supplementary routines: vector routines
 *
 *    
 *  written by Hannes H. Loeffler
 *    2010: STFC Daresbury Laboratory, UK
 *
 *****************************************************************************/

int transformOrderParameter(actionInformation *action, double *x, double *y,
			    double *z, double *box, int mode)
{
  stackType **argumentStackPointer;
  ptrajState *state;
  transformOrderParameterInfo *info;

  int i, j, k;
  int k0, k1, axis, bin, size;
  int nActive, oldActive, nActive1, nActive2;
  int *unsatMask, *atoms;

  char *mask, *buffer;

  double nstep, length, dist, prob;
  double Sx, Sy, Sz, SCD_1, SCD_2;

  dVector sx, sy, sz, cc1, cc2, a, b, t;
   ...
   *  USAGE
   *
   *  order out <filename> [unsat <mask>] [scd]
   ...
   *
   *  iarg1: axis
   *  iarg2: calculate SCD directly from C-H vectors
   *  carg1: a transformOrderParameter structure (stored in actionInformation for
   *         repeated calls)
   */
  ...
  if (mode == PTRAJ_SETUP) {
    ...
    info = (transformOrderParameterInfo *)
      safe_malloc(sizeof(transformOrderParameterInfo));
    INITIALIZE_transformOrderParameterInfo(info);
    ...
    if (info->filename == NULL) {
      fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: no output file specified. Ignoring command!\n");
      safe_free(info);
      return -1;
    }

    if (argumentStackContains(argumentStackPointer, "x")) {
      action->iarg1 = ORDPRM_X;
    } else if (argumentStackContains(argumentStackPointer, "y")) {
      action->iarg1 = ORDPRM_Y;
    } else if (argumentStackContains(argumentStackPointer, "z")) {
      action->iarg1 = ORDPRM_Z;
    } else {
      action->iarg1 = ORDPRM_Z;
    }

    /* optional end-to-end distance */
    info->tailDist =
      argumentStackKeyToString(argumentStackPointer, "taildist", NULL);

    if (info->tailDist != NULL) {
      info->delta =
	argumentStackKeyToDouble(argumentStackPointer, "delta", 0.1);

      mask = argumentStackKeyToString(argumentStackPointer, "tailstart", NULL);

      if (mask != NULL) {
	atoms = processAtomMask(mask, state);

	for (i = 0, nActive1 = 0; i < state->atoms; i++) {
	  if (atoms[i] > 0) {
	    nActive1++;
	  }
	}

	info->tailStart = safe_malloc(sizeof(int) * nActive1);

	for (i = 0, k = 0; i < state->atoms; i++) {
	  if (atoms[i] > 0) {
	    info->tailStart[k] = i;
	    k++;
	  }
	...
	fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: taildist requested but no tailstart mask specified. Ignoring command!\n");
	safe_free(info);
	return -1;
      }

      mask = argumentStackKeyToString(argumentStackPointer, "tailend", NULL);

      if (mask != NULL) {
	atoms = processAtomMask(mask, state);

	for (i = 0, nActive2 = 0; i < state->atoms; i++) {
	  if (atoms[i] > 0) {
	    nActive2++;
	  }
	}
	...
	if (nActive1 != nActive2) {
	  fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: different number of atoms in tailstart and tailend masks. Ignoring command!\n");
	  safe_free(info->tailStart);
	  safe_free(info);
	  return -1;
	}

	info->tailEnd = safe_malloc(nActive2 * sizeof(int) );
	...
	fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: taildist requested but no tailend mask specified. Ignoring command!\n");
	safe_free(info->tailStart);
	safe_free(info);
	return -1;
      }

      info->numberActiveAtoms2 = nActive2;

      info->histogram = safe_malloc(sizeof(sdDynData));
      info->histoTemp = safe_malloc(sizeof(sdDynData));
      info->histoVariance = safe_malloc(sizeof(sdDynData));

      info->histogram->size = 0;
      info->histoTemp->size = 0;
      info->histoVariance->size = 0;

      size = (int) (ORDPRM_MAXDIST / info->delta);
    ...
    /* mask for unsaturated atoms */
    mask = argumentStackKeyToString(argumentStackPointer, "unsat", NULL);

    if (mask != NULL) {
      unsatMask = processAtomMask(mask, state);
    } else {
      unsatMask = NULL;
    }

    action->iarg2 = argumentStackContains(argumentStackPointer, "scd");

    /* rest of the command line is the masks for each atom */
    info->indexes = (ordParaDynData *) safe_malloc(sizeof(ordParaDynData));

    for (i = 0;
	 (buffer = getArgumentString(argumentStackPointer, NULL)) != NULL;
	 i++) {
      info->indexes->data = (ordParaAtoms *)
	increaseMem(info->indexes->data, sizeof(ordParaAtoms), i + 2,
		    &(info->indexes->size), ORDPRM_INCSIZE);

      atoms = processAtomMask(buffer, state);

      for (j = 0, nActive = 0; j < state->atoms; j++) {
	if (atoms[j] > 0) {
	  nActive++;
	}
      }

      if (i != 0 && nActive != oldActive) {
	fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: groups are not same length!\n");
	safe_free(info->indexes->data);
	safe_free(info->indexes);
	safe_free(info);
	return -1;
      }
      ...
      info->indexes->data[i].index = safe_malloc(sizeof(int) * nActive);

      for (j = 0, k = 0; j < state->atoms; j++) {
	if (atoms[j] > 0) {
	  info->indexes->data[i].index[k] = j;

	  if (unsatMask && unsatMask[j] > 0) {
	    info->indexes->data[i].dbond = 1;
	  } else {
	    info->indexes->data[i].dbond = 0;
	  }
    ...
    if (i < 3) {
      fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: number of atoms must be at least 3 (not %i)!\n", i);
      safe_free(info->indexes->data->index);
      safe_free(info->indexes->data);
      safe_free(info->indexes);
      safe_free(info);
      return -1;
    ...
    if (action->iarg2 && ! i % 3) {
      fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: scd set but number of groups (%i) not a multiple of 3\n", i);
      safe_free(info->indexes->data->index);
      safe_free(info->indexes->data);
      safe_free(info->indexes);
      safe_free(info);
    ...
    info->numberActiveAtoms = nActive;
    info->numberGroups = i;
    info->indexes->data[i].index = NULL;

    info->_ophelper =
      safe_malloc(sizeof(double) * info->numberGroups * 3);
    info->orderParameter =
      safe_malloc(sizeof(*info->orderParameter) * info->numberGroups);

    info->_ophelper2 =
      safe_malloc(sizeof(double) * info->numberGroups * 3);
    info->ordParVariance =
      safe_malloc(sizeof(*info->ordParVariance) * info->numberGroups);

    for (i = 0; i < info->numberGroups; i++) {
      info->orderParameter[i] = info->_ophelper  + (i * 3);
      info->ordParVariance[i] = info->_ophelper2 + (i * 3);
    }
  ...
  info = (transformOrderParameterInfo *) action->carg1;
  data = info->indexes->data;
  axis = action->iarg1;

  if (mode == PTRAJ_STATUS) {
    fprintf(stdout,
            "  ORDERPARAMETER: reference axis is ");
    ...
    fprintf(stdout, "      order parameters computed from %s and written to %s\n", 
	    action->iarg2 ? "C-H vectors" : "C vectors only", info->filename);
    fprintf(stdout, "      the selected atoms are:\n");

    atoms = safe_malloc(sizeof(int) * state->atoms);

    for (i = 0; i < info->numberGroups; i++) {
      atoms = reconstructMask(data[i].index, atoms, state->atoms);

      fprintf(stdout, "        %3i) ", i + 1);
      printAtomMask(stdout, atoms, state);
    ...
    if (info->tailDist != NULL) {
      fprintf(stdout, "      tail length data written to %s\n", info->tailDist);
      fprintf(stdout, "        resolution is %4.2f A for atoms\n", info->delta);

      atoms = reconstructMask(info->tailStart, atoms, state->atoms);

      fprintf(stdout, "          start) ");
      printAtomMask(stdout, atoms, state);
      fprintf(stdout, "\n");

      atoms = reconstructMask(info->tailEnd, atoms, state->atoms);
    ...
    if (openFile(&info->outFile, info->filename, "w") == 0) {
      fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: couldn't open output file %s.\n",
              info->filename);
      return -1;
    }

    fprintf(info->outFile, "# order parameters for ");

    for (i = 0; i < info->numberGroups; i++) {
      atoms = safe_malloc(sizeof(int) * state->atoms);
      atoms = reconstructMask(data[i].index, atoms, state->atoms);

      fprintf(info->outFile, " %i) ", i + 1);
      printAtomMask(info->outFile, atoms, state);
    ...
    if (action->iarg2) {
      fprintf(info->outFile, "#Cn %10s %10s %10s %10s\n",
	      "SCD_H1", "s(SCD_H1)", "SCD_H2", "s(SCD_H2)");

      for (i = 0; i < info->numberGroups / 3; i++) {
	Sx = -info->orderParameter[i][0];
	Sy = -info->orderParameter[i][1];

	fprintf(info->outFile, "%3i %10.7f %10.7f %10.7f %10.7f\n",
		i + 1,
		Sx,
		sqrt(info->ordParVariance[i][0] / nstep),
		Sy,
		sqrt(info->ordParVariance[i][1] / nstep));
      }
    } else {
      fprintf(info->outFile, "#Cn %10s %10s %10s %10s %10s %10s %10s %10s\n",
	      "Sx", "s(Sx)", "Sy", "s(Sy)", "Sz",  "s(Sz)", "SCD_z", "SCD_xy");

      for (i = 1; i < info->numberGroups - 1; i++) {
	Sx = info->orderParameter[i][0];
	Sy = info->orderParameter[i][1];
	Sz = info->orderParameter[i][2];
	SCD_1 = 0.5 * Sz;
	SCD_2 = -(2.0 * Sx + Sy) / 3.0;

	fprintf(info->outFile, "%3i %10.7f %10.7f %10.7f %10.7f %10.7f %10.7f %10.7f %10.7f\n",
		i + 1,
		Sx,
		sqrt(info->ordParVariance[i][0] / nstep),
		Sy,
		sqrt(info->ordParVariance[i][1] / nstep),
		Sz,
		sqrt(info->ordParVariance[i][2] / nstep),
		SCD_1,
		SCD_2);
      }
    }
    ...
    if (info->tailDist != NULL) {
      if (openFile(&info->tailDistFile, info->tailDist, "w") == 0) {
	fprintf(stderr, "WARNING in ptraj(), ORDERPARAMETER: couldn't open output file %s.\n",
		info->filename);
	return -1;
      }

      fprintf(info->tailDistFile, "# end-to-end distance\n");

      for (i = 0; i < info->histogram->size; i++) {
	dist = ((double) i + 0.5) * info->delta;
	prob = info->histogram->data[i] / info->delta;

	if (prob > 0.0) {
	  fprintf(info->tailDistFile, "%10.4f %10.4f %10.7f\n", dist, prob,
		  sqrt(info->histoVariance->data[i] / nstep) );
	}
      }
    }
    ...
    for (i = 0; i < info->numberGroups; i++) {
      safe_free(info->indexes->data[i].index);
    }

    safe_free(info->_ophelper);
    safe_free(info->_ophelper2);
    safe_free(info->orderParameter);
    safe_free(info->ordParVariance);
    safe_free(info->indexes->data);
    safe_free(info->indexes);

    if (info->tailDist) {
      safe_free(info->tailStart);
      safe_free(info->tailEnd);
      safe_free(info->histogram->data);
      safe_free(info->histogram);
      safe_free(info->histoTemp->data);
      safe_free(info->histoTemp);
  ...
  if (action->iarg2) {
    for (i = 0; i < info->numberGroups; i += 3) {
      Sx = Sy = 0.0;

      for (j = 0; j < info->numberActiveAtoms; j++) {
	/* C-H1 unit vector */
	k0 =   data[i].index[j];
	k1 = data[i+1].index[j];

	vecCreate(a, x[k0], y[k0], z[k0]);
	vecCreate(b, x[k1], y[k1], z[k1]);

	vecSubtraction(sx, b, a);
	length = vecLength(sx);

	if (length > ORDPRM_MAXBOND2) {
	  fprintf(stderr, "WARNING: unusual length (> %3.1f A) of %f A. Check mask!\n",
		  ORDPRM_MAXBOND2, length);
	}

	vecScalarMult(sx, 1.0 / length, sx);
	Sx += 0.5 * (3.0 * sx[axis] * sx[axis] - 1.0);
	...
	vecScalarMult(sy, 1.0 / length, sy);
	Sy += 0.5 * (3.0 * sy[axis] * sy[axis] - 1.0);
      }

      onlineAvVar(&(info->orderParameter[i/3][0]),
		  &(info->ordParVariance[i/3][0]),
		  Sx / info->numberActiveAtoms, nstep);

      onlineAvVar(&(info->orderParameter[i/3][1]),
		  &(info->ordParVariance[i/3][1]),
		  Sy / info->numberActiveAtoms, nstep);
    }
  } else {
    for (i = 1; i < info->numberGroups - 1; i++) {
      Sx = Sy = Sz = 0.0;
	...
	vecSubtraction(sz, b, a);
	length = vecLength(sz);

	if (length > ORDPRM_MAXBOND) {
	  fprintf(stderr, "WARNING: unusual length (> %3.1f A) of %f A. Check mask!\n",
		  ORDPRM_MAXBOND, length);
	}

	vecScalarMult(sz, 1.0 / length, sz);
	...
	vecSubtraction(cc1, a, b);

	k1 = data[i+1].index[j];
	vecCreate(b, x[k1], y[k1], z[k1]);

	vecSubtraction(cc2, a, b);

	vecCrossProduct(sx, cc1, cc2);
	length = vecLength(sx);
	vecScalarMult(sx, 1.0 / length, sx);

	vecCrossProduct(sy, sz, sx);
	length = vecLength(sy);
	vecScalarMult(sy, 1.0 / length, sy);
	...
	Sx += 0.5 * (3.0 * sx[axis] * sx[axis] - 1.0);
	Sy += 0.5 * (3.0 * sy[axis] * sy[axis] - 1.0);
	Sz += 0.5 * (3.0 * sz[axis] * sz[axis] - 1.0);
      }

      onlineAvVar(&(info->orderParameter[i][0]),
		  &(info->ordParVariance[i][0]),
		  Sx / info->numberActiveAtoms, nstep);

      onlineAvVar(&(info->orderParameter[i][1]),
		  &(info->ordParVariance[i][1]),
		  Sy / info->numberActiveAtoms, nstep);

      onlineAvVar(&(info->orderParameter[i][2]),
		  &(info->ordParVariance[i][2]),
		  Sz / info->numberActiveAtoms, nstep);
    }
  }

  if (info->tailDist != NULL) {
    for (i = 0; i < info->numberActiveAtoms2; i++) {
      j = info->tailStart[i];
      k = info->tailEnd[i];

      vecCreate(a, x[j], y[j], z[j]);
      vecCreate(b, x[k], y[k], z[k]);

      vecSubtraction(t, a, b);
      bin = (int) (vecLength(t) / info->delta);

      info->histoTemp->data =
	increaseMem(info->histoTemp->data, sizeof(double), bin,
		    &(info->histoTemp->size),
		    bin - info->histoTemp->size + ORDPRM_MAXDIST);
